...
**Draw** is the user-drawn-features system. It splits into three routers (`files`, `draw`, `aggregations`) and four models (`userfiles`, `userfeatures`, `filehistories`, `published`/`publishedstore`). It is the only module gated by `s.stopGuests` — guests can read but cannot write — and it fires HTTP webhooks on changes via `Webhooks/processes/triggerwebhooks` (see the **Webhooks** entry below). Touch it when changing how user-drawn features are persisted, versioned, or aggregated. Concurrent edits coordinate through the shared Postgres rows with last-write-wins; Draw does not publish on the WebSocket — the **Config** module does (see [server bootstrap](./server-bootstrap.md)).
