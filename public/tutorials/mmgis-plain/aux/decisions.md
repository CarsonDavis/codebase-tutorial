...
The server's WebSocket connection is intentionally dumb: every message a client sends gets re-broadcast to every connected client. The receiving clients decide whether they care. The team didn't think it was worth building a smarter per-mission or per-room system; the load profile and the use case (a handful of mission operators editing a drawing together) didn't justify it.
